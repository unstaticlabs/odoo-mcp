...
| Draft vendor-bill / expense **preparatory** fields (draft-only; expense amount via `total_amount` — `total_amount_currency` is audit-only; expense payer ("Paid By") via `payment_mode` (`own_account` / `company_account`); vendor-bill review-queue status via `review_state`) | `billing.update_draft_expense`, `billing.configure_draft_vendor_bill` |
...
| **Copy an existing attachment / Documents file onto a canonical draft vendor bill** (de-duplication: the surviving bill keeps the evidence, the zero-value shell can then be deleted) | `billing.copy_or_relink_source_attachment` (write; draft `in_invoice` only; `/mcp` + `/accounting/mcp`). `mode: copy` (default) creates a **new** `ir.attachment` with the same bytes and leaves the source untouched; `mode: relink` repoints the `documents.document` filing instead (**destructive** to the previous link, needs `source_document_id` + the Documents app). Supply exactly one of `source_attachment_id` / `source_document_id`. Refuses URL-only and oversize sources; reports the copy's `checksum` / `file_size`. Not generic `ir.attachment` CRUD |
| **Record a draft vendor receipt** (what physically arrived, as close-time evidence) | `inventory.create_draft_vendor_receipt` (write; `/mcp` + `/accounting/mcp`; one incoming `stock.picking` with nested `move_ids`, left **unvalidated**). `dry_run: true` returns the exact planned vals with zero writes. Destination must be an internal location; `picking_type_id` is resolved from `code=incoming` when omitted. Exposes **no** validate path — `button_validate` / `action_validate` stay human-only and remain denied on the generic tools |
...
2. `billing.update_draft_expense` / `billing.configure_draft_vendor_bill` — draft preparatory fields only.
   ...
   Expense payer prep uses `payment_mode` — Odoo's **Paid By** — (`own_account` / `company_account`) on draft expenses only —
...
still refused with `draft_required` first, and `HARD_DENY_FIELDS` (`state`, `payment_state`,
`journal_id`, `account_move_id`, …) stay blocked. `payment_mode` ("Paid By") is *not* hard-denied —
it is an allowlisted draft-expense prep field (still refused on vendor-bill headers and invoice lines).
...
ACLs. The only dedicated `inventory.*` tool is `inventory.create_draft_vendor_receipt` (draft incoming
`stock.picking` + its moves, ODOO2298) — master-data graduation is unchanged by it, and every other
`product.*` / `stock.*` write, including validating a receipt, stays default-denied. The list is deliberately *not*
