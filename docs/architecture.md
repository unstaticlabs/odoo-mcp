...
- `/mcp` is the broad default static view, capped at 30 tools and `DEFAULT_PROFILE_SCHEMA_TOKEN_BUDGET` (15,500) estimated schema tokens; it omits deferred-loading hints so hosted clients receive every listed schema. Actual client retention must be verified after reconnecting.
