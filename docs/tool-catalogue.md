...
- The flag is optional rather than defaulted, and omitting it is valid only for plain text. A body carrying recognised HTML tags with no explicit flag is refused in preflight as `markup_declaration_required`, before any Odoo call. Escaping it instead would succeed silently and store a record an Agent identity cannot repair, since `mail.message` is not writable by an Agent. An explicit `false` is still honoured, so prose that merely looks tag-shaped stays postable.
- Markdown is rendered in neither mode, and plain text sent with the flag true loses its line breaks. Both return a `warnings` entry rather than a refusal.
