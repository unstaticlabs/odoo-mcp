...
- `MCP_DOCUMENT_MATERIALIZATION_ENABLED=true` for the coordinated Distribution backend and audited document ingress;
- exact 40-character Git revisions in `MCP_BUILD_ID` and `MCP_GITOPS_COMMIT` so feedback reports carry immutable MCP and GitOps identities;
...
6. Require `GET /readyz` to return `status=ready`, the default tool budget within 31/16,500, OAuth `ready` or deliberately `disabled`, and analytics `ready` or deliberately `disabled`. Analytics `degraded` does not make the MCP unavailable, but fix it before treating telemetry as complete.
