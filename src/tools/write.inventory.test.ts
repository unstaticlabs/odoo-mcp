/**
 * Narrow inventory master-data graduation (card ODOO2240) — tool-level contract.
 *
 * `product.category` and `stock.location` create/write reach Odoo under the caller's ACLs; every
 * refusal on those paths (connector policy, duplicate, Odoo ACL, Odoo validation) is a structured
 * envelope naming the refusing layer and the next step — never an opaque error. Sibling
 * `product.*` / `stock.*` models stay default-denied.
 */
