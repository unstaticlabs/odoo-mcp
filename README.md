...
| `documents.list_saved_views` | read | `query`, `scope` (`all` \| `shared` \| `personal`), `limit` (1–100), `offset` — shared and caller-owned Documents views, including their safe structured filters and catalog facets; IDs feed `documents.search` and `documents.find_similar` |
