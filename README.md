...
| `projects.update_task` | write | `task_id` (positive int) plus at least one of `name`, `description`, `description_is_html` (bool, default `false`), `date_deadline` (`null` clears), `stage_id`, `priority` (`"0"`/`"1"`), `context` (optional) — curated `project.task` `write`; plain text is HTML-escaped for you; no free-form `values` dict, and an empty update is refused before any Odoo call |
